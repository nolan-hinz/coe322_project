...
int main( int argc, char ** argv ) {
  ...
  // WAS NOT ABLE TO MAKE THESE FEATURES WORK IN TIME
  /*  options.add_options()
  ...
  options.add_options()("ts,track_sardines","<bool> -ts if you want to track the sardine population",
			cxxopts::value<bool>()->default_value("0"));
  options.add_options()("sp,sardine_params","<double,double,double>, initial sardine population, rate at which sardines reproduce, percent of sardines eaten before reproduction happens.",
			cxxopts::value<vector<double>>()->default_value("100,.2,.5"));
  */					 
  // Parse the input
  ...
  double sardine_birth_rate = .2;
  double sardine_eaten_rate = .5;
